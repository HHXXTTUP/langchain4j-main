...
13. **GPT Images 资产生成协议**：剧本设定阶段可为每个角色生成纯白背景、正面全身的基础人物图，并将人物名称写入画面；过审档案图必须使用横向 16:9 画布、顶部独立姓名栏和 6×3 多格档案排版。只要格子内出现可辨认人脸或局部五官，就必须放置一条纯黑、不透明、边缘平直的横向长矩形遮挡条；无法合理遮挡时必须改成无脸的服装、手部或背部细节。七类遮挡必须在整张图中各出现且只出现一次：第1格正面全身遮眉眼，第2格正面半身遮鼻梁与鼻尖，第3格左侧面遮嘴唇，第4格右侧面遮下巴与下颌线，第5格三分之二侧面遮左脸颊与颧骨，第8格回头侧脸遮右脸颊与耳前区域，第9格面部近景遮额头下缘与发际线。第6、7、10至18格只展示背面、后侧发型、颈肩领口、耳后、手部、服装或鞋履等无清晰面部内容，不加黑条。每个含脸格只能有一条黑条且只能遮指定部位，七类位置不得在其他格重复，尤其禁止任意两个格同时遮眼。黑条长度为可见脸部宽度的65%至85%，高度为12%至22%，不得跨格或变成短方块。整图必须分散保留眼、鼻、嘴与下颌线，使不同格未遮挡区域可互补拼出同一张完整统一的人脸。禁止遮整张脸、遮整个人头、马赛克、模糊和渐变；输出前必须按格号检查七个唯一位置、无脸格和多余黑块。剧集复刻阶段必须以基础图为第一参考图，结合本集可编辑的服装、发型、配饰和状态提示词调用图生图，直接输出同样版式的过审档案图；只替换明确要求变化的服装资产，身份、脸型、五官、人体比例和未要求变化的资产保持锁定。环境参考图单独依据本集环境资料调用文生图，不得生成人物。所有图片结果必须落盘并提供查看/下载，后续段落自动复用已保存资产；不得把图片接口改成聊天接口。
...
15. **分集标题与概述协议**：每次首次生成、续写或重写一集，必须同时输出本集标题、内容概述和正文。标题用8至18个汉字概括该集核心冲突或反转，不能只写“第N集”，不能复用上一集标题；内容概述用60至120个汉字交代起因、主要冲突、关键变化和结尾钩子，不得包含分镜、运镜、时长等视频制作术语。标题与概述必须和正文一起持久化，在“我的剧本”中直接展示；刷新页面只能读取已保存内容，不得重新请求模型生成。
