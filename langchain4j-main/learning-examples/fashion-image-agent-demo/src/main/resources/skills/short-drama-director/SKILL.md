...
13. **GPT Images 资产生成协议**：剧本设定阶段可为每个角色生成纯白背景、正面全身的基础人物图，并将人物名称写入画面；过审档案图必须使用横向 16:9 画布、顶部独立姓名栏和参考图一致的多格档案排版。只要一个格子出现任何可辨认人脸或局部五官，包括正脸、侧脸、回头脸、眼睛、眉毛、鼻子、嘴唇、脸颊、耳前脸部或下颌线，就必须放置一条纯黑、不透明、边缘平直清晰的横向长矩形遮挡条；不允许清晰人脸局部无黑条。若局部特写无法合理放置黑条，必须改为无脸的服装、手部或背部细节。黑条长度约为该格可见脸部或五官区域宽度的55%至80%，高度约为该区域高度的12%至22%，至少覆盖可识别区域的一半但不能跨出当前格；不得生成短小方块。眼部眉形格必须遮眉眼或眼部主体，鼻唇局部格必须遮鼻梁至上唇或口鼻主体；其他脸部格的黑条位置在眉眼、鼻梁至上唇、口鼻至脸颊前缘、单眼至鼻梁、下半脸或嘴角间轮换，相邻格不得完全相同。生成前逐格检查：任何含可见眼、眉、鼻、口、脸颊或完整脸但无独立黑条的格子必须删除或改为无脸细节。禁止黑条跨格、遮住整张脸或整个人头、重复统一遮双眼、马赛克、模糊和渐变；不同角度的未遮挡区域必须足以拼合识别同一张完整脸。剧集复刻阶段必须以基础图为第一参考图，结合本集可编辑的服装、发型、配饰和状态提示词调用图生图，直接输出同样版式的过审档案图；只替换明确要求变化的服装资产，身份、脸型、五官、人体比例和未要求变化的资产保持锁定。环境参考图单独依据本集环境资料调用文生图，不得生成人物。所有图片结果必须落盘并提供查看/下载，后续段落自动复用已保存资产；不得把图片接口改成聊天接口。
