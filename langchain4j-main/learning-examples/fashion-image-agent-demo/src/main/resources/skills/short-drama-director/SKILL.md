...
10. **复刻段落独立连续性协议**：每一个可单独交给视频模型的段落必须是一段完整中文描述，来自当前集不同的剧情推进；不得复制其他段落，不得输出分镜编号、时间轴、时长字段、规则解释或空泛形容词。每段必须独立写清当前出场人物及本段服装装束、当前环境/空间锚点/场面氛围、构图与景别、视角与焦段感、摄影机运镜、清晰度/光线/色彩/景深、可观察的表情生理变化、动作因果与受力、视线落点、对白双引号和字幕、声音节奏，以及本段结尾如何把动作、光源、轴线和情绪自然交给下一段。禁止输出“同上一段一致”“同上一集一致”“上一段结尾连续性参考”等内部占位语句，禁止粘贴上一段正文。段落之间要像同一场连续拍摄，独立生成后仍可无明显断层合并。每段适合30秒以内的口播/拍摄节奏；若视频模型限制15秒则按15秒生成，不能为了凑段数复制内容。
11. **复刻请求数据契约**：一次模型请求必须返回一个JSON对象，顶层只含 `episodeMaterial` 与 `segments`。`episodeMaterial` 只在复刻页面顶部展示并持久化，包含本集人物装束、环境氛围、主要剧情和连续性；`segments` 中每项包含当前段落正文以及人物、环境、摄影机、画面质感、表演动作、对白声音、衔接字段。服务端将这些字段合并成一段可直接投喂的中文提示词；页面刷新只读数据库，不得再次调用模型。
