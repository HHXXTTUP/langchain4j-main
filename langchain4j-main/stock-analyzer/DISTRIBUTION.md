...
生成新的盯盘授权令牌（参数为到期日 `yyyyMMdd`）：

```powershell
mvn test-compile
java -cp target\test-classes`;target\classes dev.learning.stockanalyzer.service.MonitorLicenseTokenGenerator 20260915 .private\monitor-license-ed25519.key
```

命令会直接输出一行 `stock.license.monitor-expiry-ciphertext=...`。把这一整行
替换到 `packaging\stock-lens.properties` 后再构建分发包。该字段就是加密后的
盯盘到期时间；到期日当天仍可使用，次日起不可用。
私钥文件位于 `.private`，已被 Git 忽略；不要把它放入源码仓库或分发包。
