...
$portableReadme = Join-Path $projectRoot 'PACKAGED-CLEAN-README.txt'
...
$portableConfiguration = Join-Path $projectRoot 'packaging\stock-lens-clean.properties'
...
try {
    ...
    $jpackageArgs = @(
        ...
        '--description', 'A-share public market data and company information viewer',
        '--input', $stagingRoot,
        ...
        '--arguments', '--spring.profiles.active=desktop,public-clean'
    )
    ...
    $configurationDirectory = Join-Path $appImage 'config'
    New-Item -ItemType Directory -Path $configurationDirectory -Force | Out-Null
    Copy-Item -LiteralPath $portableConfiguration -Destination (Join-Path $configurationDirectory 'stock-lens.properties')
